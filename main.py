...
train_player_old = [random.randint(0,2), random.randint(0,2), random.randint(0,2)]
best_player = None
train_player_move = 0
...
def eval_genomes_with_player(genomes, config):
    ...
    ge = []
    nns = []
    print(train_player_old)
    if best_player is not None :
        net = neat.nn.FeedForwardNetwork.create(best_player, config)
        nets.append(net)
        nns.append(Player(0))
        ge.append(best_player)
    for _, g in genomes :
        net = neat.nn.FeedForwardNetwork.create(g, config)
        nets.append(net)
        nns.append(Player(0))
    ...
    print('Number of nets : {}'.format(len(nets)))

    while True:
        player_ohe = np.zeros((3,3))
        player_ohe[0, train_player_old[-1]] = 1
        player_ohe[1, train_player_old[-2]] = 1
        player_ohe[2, train_player_old[-3]] = 1
        if len(nns) == 0:
            break
        
        for x, nn in enumerate(nns):            
            output = nets[x].activate((
                player_ohe[0, 0], player_ohe[0, 1], player_ohe[0, 2],
                player_ohe[1, 0], player_ohe[1, 1], player_ohe[1, 2], 
                player_ohe[2, 0], player_ohe[2, 1], player_ohe[2, 2] 
            ))
            move = np.argmax(output)
            nn.play_move(move)

            result = mat_games[nn.move, train_player_move]
...
def training_with_player(config, winner_filename):
    global train_player_move
    global best_player
    # Create population based on the config we set
    p = neat.Population(config)
...
def play_trained_network(winner, config):
    ...
    while True:
        ...
        player_ohe = np.zeros((3,3))
        player_ohe[0, player_old[-1]] = 1
        player_ohe[1, player_old[-2]] = 1
        player_ohe[2, player_old[-3]] = 1
...
if __name__ == '__main__':
    ...
    # Training network 
    training_with_player(config, winner_filename)
